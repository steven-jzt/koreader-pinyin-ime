...
function VirtualKeyboard:init()
    ...
    self.height = Screen:scaleBySize(keys_height * #self.KEYS + 40) -- +40 给候选栏
    ...
    -- 拼音输入法：注册候选栏刷新回调
    if self.inputbox and self.inputbox._phone_ime then
        self.inputbox._phone_ime:setUpdateCallback(function(candidates, index, code)
            self:updateCandidateBar(candidates, index, code)
        end)
    end
...
function VirtualKeyboard:addKeys()
    ...
    local candidate_bar_height = 40
    local base_key_height = math.floor((self.height - candidate_bar_height - (#self.KEYS + 1)*self.key_padding - 2*self.padding)/#self.KEYS)
    local h_key_padding = HorizontalSpan:new{width = self.key_padding}
    local v_key_padding = VerticalSpan:new{width = self.key_padding}
    local vertical_group = VerticalGroup:new{ allow_mirroring = false }
    -- 拼音候选栏（初始为空，有输入法时填充）
    self.candidate_bar = TextWidget:new{
        text = "",
        face = Font:getFace("cfont", 20),
        bold = true,
        padding = Size.padding.small,
        max_width = self.width - 2 * Size.border.default - 2 * self.padding,
    }
    self.candidate_bar_container = LeftContainer:new{
        dimen = Geom:new{
            w = self.width - 2 * Size.border.default - 2 * self.padding,
            h = self.candidate_bar:getSize().h,
        },
        self.candidate_bar,
    }
    table.insert(vertical_group, self.candidate_bar_container)
    table.insert(vertical_group, v_key_padding)
...
function VirtualKeyboard:updateCandidateBar(candidates, index, code)
    if not self.candidate_bar_container then return end
    local parts = {}
    if code and code ~= "" then
        table.insert(parts, code) -- 显示拼音字母
    end
    ...
    -- 重建 TextWidget（比 setText 可靠，绕开 free/重渲染失效的问题）
    ...
    self.candidate_bar_container[1] = self.candidate_bar
    self:_refresh(true)
end
